...
namespace tmx
{
    ...
    */
    ...
    support Text nodes.
    */
    class TMXLITE_EXPORT_API MapObject final
    {
    public:
        enum class Shape
        {
        ...
        read the MapObject properties if it is valid.
        */
        void parse(const pugi::xml_node&, Map*);

        /*!
        \brief Returns the unique ID of the MapObject
        */
        std::uint32_t getUID() const { return m_UID; }

        /*!
        \brief Returns the name of the MapObject
        */
        const std::string& getName() const { return m_name; }

        /*!
        \brief Returns the type of the MapObject, as defined in the editor
        */
        const std::string& getType() const { return m_type; }

        /*!
        \brief Returns the position of the MapObject in pixels
        */
        ...
        define the MapObject's width and height. This can be used to derive
        the shape of the MapObject if it is rectangular or elliptical.
        */
        const FloatRect& getAABB() const { return m_AABB; }

        /*!
        \brief Returns the rotation of the MapObject in degrees clockwise
        */
        float getRotation() const { return m_rotation; }

        /*!
        \brief Returns the global tile ID associated with the MapObject
        if there is one. This is used to draw the MapObject (and therefore
        the MapObject must be rectangular)
        */
        uint32_t getTileID() const { return m_tileID; }

        /*!
        \brief Returns whether or not the MapObject is visible
        */
        bool visible() const { return m_visible; }

        /*!
        \brief Returns the Shape type of the MapObject
        */
        Shape getShape() const { return m_shape; }

        /*!
        \brief Returns a reference to the vector of points which
        make up the MapObject. If the MapObject is rectangular or elliptical
