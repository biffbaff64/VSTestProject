...
#include "Map.hpp"
#include "Tileset.hpp"
#include "TMX/detail/pugixml.hpp"
...
//public
void MapObject::parse(const pugi::xml_node& node, Map* map)
{
    std::string attribString = node.name();
    if (attribString != "object")
    {
        Logger::log("This not an MapObject node, parsing skipped.", Logger::Type::Error);
        return;
    }
...
//private
void MapObject::parsePoints(const pugi::xml_node& node)
{
...
void MapObject::parseText(const pugi::xml_node& node)
{
...
void MapObject::parseTemplate(const std::string& path, Map* map)
{
    assert(map);
    ...
    if (templateObjects.count(path) == 0)
    {
        ...
        if (obj)
        {
            templateObjects.insert(std::make_pair(path, MapObject()));
